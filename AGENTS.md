...
Streaming (`append(_:type:)` / `write(_:)`) keeps `elementEndOffsets == nil` and never allocates structure. The table is materialized by the first append whose element is not exactly one non-empty atom, at 8 bytes per element (versus 104 bytes per token for the old boxed element tree). A component that flattens to nothing — an appended `nil` optional, `EmptyComponent`, an empty composite — records a **zero-length element** (`ends[i] == ends[i-1]`): no atoms, no text, but it occupies an element slot, exactly as it did in the historical element tree.

**Two invariants.** Break either and the same content starts comparing or rendering differently depending on how it was built:

1. **Granularity is the boundary table.** One appended component is one element, whatever its atom count — including third-party leaves whose `buildComponents()` returns several atoms. `append(_ semanticString:)` splices element-wise. The only entry point that records no element for empty input is `append(_:type:)` with an empty string, matching its historical behavior.
2. **No zero-length atoms in `atoms`, ever, on any path.** `components` returns the array verbatim, so every write entry point drops empty atoms (`droppingZeroLengthComponents(_:)`, the append funnel, eager composite flattening). Filtering on *read* instead would make `components` O(n) with an allocation. Zero-length *elements* are recorded; zero-length *atoms* are not.

**The element view.** `SemanticString.elements` returns `SemanticStringElements` — either one contents run (`atoms` + boundary table) or an array of independent strings, both zero-copy. An element's components are always a contiguous run of the flat array, so containers read them as `ArraySlice`s via `atoms(ofElementAt:)`: no scratch buffers, no per-item array materialization, no boxing. Measured, container flattening is faster than the pre-redesign element tree ever was.
...
- **`isEmpty` means "has no elements", O(1).** A zero-output element (`EmptyComponent`, an appended `nil` optional, an empty composite) still counts, exactly as it occupied a slot in the historical element tree. `count == 0` is the "renders nothing" measure. `ForEach(_:separator:)` uses `isEmpty` per item, which is what makes its output byte-identical with the pre-storage-change library — including the separator it emits around an item that renders nothing.
- **Identifier scopes stamp `append(_:type:)` / `write(_:type:)` only.** Component appends never stamp; a component that wants an identifier carries its own via `AtomicComponent(string:type:identifier:)`. Every `appending` result drops the receiver's open scopes, including the empty-string early returns.
- **The zero-allocation append path is opt-in via `PlainAtomicSemanticComponent`.** `append` has four component overloads: exact `AtomicComponent` (keeps `identifier`), `some PlainAtomicSemanticComponent` (statically resolved, reads `string` / `type` straight into the atom array, so `Keyword` / `Space` / `Indent` / `TypeName` never allocate), `some AtomicSemanticComponent` (goes through `buildComponents()`), and `some SemanticStringComponent` (flattens eagerly). Every overload records exactly one element, so granularity cannot diverge between paths — however many atoms a custom `buildComponents()` returns, they stay one element.
