...
- `Span.length` is a `UInt16`, so a token longer than 65535 UTF-8 bytes is split into consecutive spans carrying the same type and identifier, at grapheme cluster boundaries — no span slices a `Character` (an emoji ZWJ sequence, a combining-mark cluster) in half. A single cluster that itself exceeds 65535 bytes degrades to Unicode scalar boundaries. Consumers that concatenate text see no difference; a token-by-token count comparison against `SemanticString.components` will.
