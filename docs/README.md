...
- [AppendPathPerformanceExplained.md](AppendPathPerformanceExplained.md) —— 上一篇的讲解版，面向任何 Swift 开发者：每项优化改了什么、为什么成立（`String` 的 UTF-8 表示、存在量与协议见证表、result builder 的展开与参数所有权、数组的写时复制）、被放弃的更快方案及理由、怎么证明行为没变。
