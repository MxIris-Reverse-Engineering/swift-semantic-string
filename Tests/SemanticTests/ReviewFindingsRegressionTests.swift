...
struct ReviewFindingsRegressionTests {
    ...
    @Test("Plain leaves still take the flat path without boxing")
    func plainLeavesStayFlat() {
        ...
        #expect(streamed._storage.treeElements.isEmpty)
        #expect(streamed.count == 4)
    }

    // MARK: - Element Views Must Not Box Flat Atoms

    @Test("A flat string's element view holds the typed array, not existentials")
    func flatElementViewDoesNotBox() {
        // Materializing `[any SemanticStringComponent]` here costs a 40-byte
        // container plus a 64-byte heap box per atom — measured at +79 MB and
        // +23 ms for a 400k-atom string handed to `MemberList(level:content:)`,
        // which is more than the whole two-state change saves.
        var streamed = SemanticString()
        for index in 0 ..< 8 {
            streamed.append("token\(index)", type: .standard)
        }

        guard case .flat(let components) = streamed.elements.representation else {
            Issue.record("a streamed string's element view should stay flat")
            return
        }
        #expect(components.count == 8)
    }

    @Test("Container prebuilt-content initializers keep the flat representation")
    func containersDoNotBoxPrebuiltFlatContent() {
        var streamed = SemanticString()
        for index in 0 ..< 4 {
        ...
        guard case .flat = memberList.items.representation else {
            Issue.record("MemberList(level:content:) boxed a flat string's atoms")
            return
        }

        let rows = Rows(level: 1, content: streamed)
        guard case .flat = rows.items.representation else {
            Issue.record("Rows(level:content:) boxed a flat string's atoms")
            return
        }

        let blockList = BlockList(content: streamed)
        guard case .flat = blockList.items.representation else {
            Issue.record("BlockList(content:) boxed a flat string's atoms")
            return
        }
    }
    ...
    @Test("Element granularity is per-atom when flat and per-item when built")
    func elementGranularityMatchesHowTheStringWasBuilt() {
        // The two granularity tests in `TwoStateStorageTests` both go through a
        // builder closure, and `buildFinalResult` makes one element per
        // `SemanticString` regardless of storage state — so they hold even if
        // the two-state design is reverted wholesale. These assertions do
        // depend on it.
        var streamed = SemanticString()
        ...
        // container's prebuilt-content initializer. That is the reason
        // `compact()` stays internal.
    ...
    // MARK: - isEmpty Must Not Materialize or Publish

    @Test("isEmpty on a tree does not publish a flattened array into the cache")
    func isEmptyDoesNotPublishTheCache() {
        // `isEmpty` used to go through `components`, flattening the whole tree
        // and leaving the result in storage that may be shared — so a value
        // that never asked for it ended up holding a full `[AtomicComponent]`.
        // `ForEach(_:separator:)` calls this once per item.
        let built = SemanticString {
            Group { Keyword("a") }
            Group { Keyword("b") }
        }
        #expect(built._storage.cachedComponents == nil)
    ...
    @Test("isEmpty on a shared copy does not inflate the original")
    func isEmptyOnACopyDoesNotInflateTheOriginal() {
    ...
    @Test("isEmpty still reports composites that flatten to nothing")
    func isEmptyReportsCompositesThatFlattenToNothing() {
        let empty = SemanticString {
            Group {}
            Group {}
        }
        #expect(!empty._storage.isFlat)
        #expect(empty._storage.treeElements.count == 2)
